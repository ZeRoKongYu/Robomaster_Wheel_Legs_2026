...
float PID_Calculate(PID_t *pid, float measure, float ref)
{
    ...
    pid->Ref = ref;
		...
		if(pid->Pid_Mode == ANGLE)
		{
			if(pid->Measure>0 && pid->Ref<0)
			{
				a=360-pid->Measure+pid->Ref;
				b=pid->Ref-pid->Measure;
				(a<-b)?(pid->Err=a):(pid->Err=b);
			}
			else if(pid->Measure<0 && pid->Ref>0)
			{
				a=-360-pid->Measure+pid->Ref;
				b=pid->Ref-pid->Measure;
				(-a<b)?(pid->Err=a):(pid->Err=b);
		...
		else if(pid->Pid_Mode == RADIAN)
		{
			if(pid->Measure>0 && pid->Ref<0)
			{
				a=2*PI-pid->Measure+pid->Ref;
				b=pid->Ref-pid->Measure;
				(a<-b)?(pid->Err=a):(pid->Err=b);
			}
			else if(pid->Measure<0 && pid->Ref>0)
		...
		else if(pid->Pid_Mode == NO_CIRCLE)
		{
			pid->Err = pid->Ref - pid->Measure;
		}
