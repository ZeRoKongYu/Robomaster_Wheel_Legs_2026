...
void Receive_Motor_Data(motor_fbpara_t *motor_fbpara, uint8_t *rx_data,uint8_t model)
{
	...
	{
		if(motor_fbpara->id == 3)
		{
			motor_fbpara->pos = Half_Circle_RADIAN(uint_to_float(motor_fbpara->p_int, P_MIN_8009, P_MAX_8009, 16) - 0.0226264f);
		}
		else if(motor_fbpara->id == 4)
		{
			motor_fbpara->pos = Half_Circle_RADIAN(uint_to_float(motor_fbpara->p_int, P_MIN_8009, P_MAX_8009, 16) + 0.0016298f);
		}
		else
		{
